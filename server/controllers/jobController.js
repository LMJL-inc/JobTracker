...
module.exports = {
    ...
    async addJob(req, res, next) {
        try {
            ...
            const data = await Jobs.findOne({ username: username, companyName: companyName, jobTitle:jobTitle })
            if (!data) {
    ...
    async updateJobStatus(req, res, next) {
        ...
        try {
            const updatedJob = await Jobs.updateOne({ _id: _id }, { status:status });
            return next()
        } catch(err) {
            return next({
    ...
    async updateJobDetails(req, res, next) {
        ...
        try {
            ...
            await Jobs.updateOne({_id: _id }, req.body);
            return next();
        } catch(err) {
            return next({
